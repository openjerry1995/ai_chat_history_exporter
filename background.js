...
// ── PLATFORM REGISTRY ── (所有平台逻辑封装在此，主流程零平台代�?
const PLATFORM_REGISTRY = {
  chatgpt: {
    ...
    getItems: () => {
      ...
      return Array.from(links).map(a => {
        ...
        if (t.length > 1 && !t.includes('新聊�?) && !t.includes('Search') && a.offsetParent) {
  ...
  grok: {
    extractCurrent: () => {
      const META = ['毫秒','快速模�?,'畅所欲言','privacy','cookie','解释chat history','推荐聊天记录','�?think','新建聊天','语音','imagine'];
      const main = document.querySelector('main');
    ...
    openSidebar: () => {
      ...
      let sidebarToggle = document.querySelector('[aria-label*="sidebar" i], [aria-label*="侧边�?], [aria-label*="sidebar"]');
      if (!sidebarToggle) {
        ...
        for (const btn of allBtns) {
          ...
          if (label.includes('sidebar') || label.includes('侧边�?) || text.includes('sidebar') || text.includes('侧边�?)) {
            sidebarToggle = btn;
            break;
          }
        }
